...
def inference():
    try:
        ...
        data = {}
        data.update({'candidates': model_inference.inference(sentence, request_cnt, "BibleLM/Bible_model_ckpts") if request_cnt > 0 else []})
        data.update({"success": True})
    except Exception as e:
        data = {"success": False, 'error' : str(e), 'cmd':ssubprocess.run('ls', shell=True, cwd=params['cwd'] , capture_output=True) }
